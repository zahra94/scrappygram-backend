...
if ($route == "dashboard") {
	if($isAdmin) {
		...
		$openTickets = $database->select("tickets", "*", [ "status[!]" => "Closed" ]);
		...
		$openTickets = $database->select("tickets", "*", [ "AND" => ["clientid" => $liu['clientid'], "status[!]" => "Closed"]] );
...
// REPORTS
if ($route == "reports") { isAuthorized("viewReports"); $clients = getTable("clients"); $admins = getTableFiltered("people","type","admin"); $pageTitle = __("Reports"); }
...
if ($route == "people/staff") { 
	...
	$admins = getTableFiltered("people","type","admin"); $pageTitle = __("Staff");
 }
...
if ($route == "customers") {
	...
	$customers = getTable("customers");
	$pageTitle = __("Customers");
}

if ($route == "customers/edit") {
	// isAuthorized("editCustomer");
	$customer = getRowById("customers",$_GET['id']);
