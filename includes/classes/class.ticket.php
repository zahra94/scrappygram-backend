...
class Ticket extends App {
    ...
    public static function emailToTicket($from,$subject,$body,$importance,$ccs){
    	...
    	if($data['userid'] == 0) $data['clientid'] = 0;
    		...
    		if(!empty($asset['id'])) $data['assetid'] = $asset['id'];
    	}
    	...
    	foreach($tickets as $ticket) {
    		if (strpos($subject,$ticket['ticket']) !== false) { $data['ticketid'] = $ticket['id']; $data['notification'] = false; break; } else { $data['ticketid'] = 0; $data['notification'] = getConfigValue("tickets_notification"); }
    	}

    	if(empty($tickets)) { $data['ticketid'] = 0; $data['notification'] = getConfigValue("tickets_notification"); }

    	if($data['ticketid'] == 0) self::add($data); else self::addReply($data);
